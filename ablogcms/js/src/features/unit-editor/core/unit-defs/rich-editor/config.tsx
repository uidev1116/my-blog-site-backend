...
import AsyncRichEditor from './components/async-rich-editor';
  ...
  return (
    <VStack spacing="1rem" align="stretch">
      ...
      <Grid>
        ...
        <GridItem col={{ xs: 12, sm: 10 }}>
          <AsyncRichEditor
            html={config.field_1 ? JSON.parse(config.field_1).html : ''}
            onChange={(value) =>
              editor.update(config.id, (config) => ({ ...config, field_1: JSON.stringify(value) }))
            }
          />
        </GridItem>
      </Grid>
